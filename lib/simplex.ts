...
export function fbmSimplex(x: number, y: number, z: number, octaves: number, lacunarity = 2.0, persistence = 0.5): number {
  let value = 0
  let amplitude = 1
  let frequency = 1
  let total = 0
...
// Ridged noise - creates sharp ridge features
export function ridgedNoise(x: number, y: number, z: number, octaves: number): number {
  let value = 0
  let amplitude = 1
  let frequency = 1
  let total = 0
